...
(function initMuiBridge() {
    ...
    function normalizeRelatorioTotalFilters(filters, tipoId) {
        ...
        return filters
            ...
            .filter((filter) => {
            if (!filter.key || !filter.enabled)
                return false;
            if (filter.key === 'sum_of_sums')
                return true;
            if (!filter.key.startsWith('attr_total:'))
                return false;
    ...
    function saveReportConfig(payload) {
        ...
        const existing = state.reportConfigs.find((item) => ((savedId && item.id === savedId) ||
    ...
    function generateReport(payload) {
        ...
        const sumOfSumsValue = somarNumericos && sumOfSumsEnabled
            ? numeroTotals.reduce((acc, total, idx) => {
                ...
                if (!attr || total === null)
                    return acc;
                if (!sumOfSumsAttrSet.has(attr.id))
                    return acc;
